...
const STATE_KEY = 'fisica_avance_pwa_v6';
...
function setupExportImport(){
  ...
  if(btnExp){
    btnExp.onclick = () => {
      ...
      a.download = 'progreso-profesorado-fisica.json';
...
function setupBuscador(){
  ...
  input.addEventListener('input', ()=>{
    const q = input.value.trim().toLowerCase(); ul.innerHTML=''; if(!q) return;
...
function isHabilitada(m, state){
  ...
  const allCursada = (arr) => arr.every(x => (typeof x==='number') ? haveCursada(x) : (x&&x.anyOf) ? x.anyOf.some(id => haveCursada(id)) : true);
  const allAcreditar = (arr) => arr.every(x => (typeof x==='number') ? haveApproved(x) : (x&&x.anyOf) ? x.anyOf.some(id => haveApproved(id)) : true);
...
function renderChecklist(){
  ...
  const state = loadState(); cont.innerHTML = '';
  materias.forEach(m => {
  ...
  materias.forEach(m => {
    ...
    box.onclick = () => {
      if(st.clase === 'bloqueada'){ mostrarBloqueo(m, state); }
        ...
        const n = parseFloat(val); const s = loadState();
        if(!Number.isNaN(n)){ if(!s.aprobadas[m.id]) s.aprobadas[m.id] = {}; s.aprobadas[m.id].nota = n; saveState(s); renderChecklist(); renderMatriz(); renderProgreso(); }
      }else{
        const cur = state.aprobadas[m.id]?.nota ?? '';
        const val = prompt(`Cosas del 41 te recomienda que al editar nota para “${m.nombre}” (actual: ${cur}). Borrar para eliminar.`, cur); if (val === null) return;
        const s = loadState(); if (val.trim()===''){ delete s.aprobadas[m.id]; } else { const n = parseFloat(val); if(!Number.isNaN(n)){ if(!s.aprobadas[m.id]) s.aprobadas[m.id] = {}; s.aprobadas[m.id].nota = n; } }
        saveState(s); renderChecklist(); renderMatriz(); renderProgreso();
      }
    };
...
// === Bloqueo (modal con nombres y lista) ===
function requisitosFaltantesNombres(m, state){
  const haveApproved = (id) => { const mm = materias.find(x=>x.id===id); return mm ? isAprobada(mm, state) : false; };
  const haveCursada = (id) => { const mm = materias.find(x=>x.id===id); return mm ? hasCursada(mm, state) : false; };
  const falt = [];
  const pushIfMissingC = (token)=>{
    if (typeof token === 'number'){ if (!haveCursada(token)){ const mm = materias.find(x=>x.id===token); if(mm) falt.push(mm.nombre); } }
    else if (token && token.anyOf){ const ok = token.anyOf.some(id => haveCursada(id)); if (!ok){ const names = token.anyOf.map(id => (materias.find(x=>x.id===id)||{}).nombre).filter(Boolean); falt.push('al menos una de: ' + names.join(', ')); } }
  };
  const pushIfMissingA = (token)=>{
    if (typeof token === 'number'){ if (!haveApproved(token)){ const mm = materias.find(x=>x.id===token); if(mm) falt.push(mm.nombre); } }
    else if (token && token.anyOf){ const ok = token.anyOf.some(id => haveApproved(id)); if (!ok){ const names = token.anyOf.map(id => (materias.find(x=>x.id===id)||{}).nombre).filter(Boolean); falt.push('al menos una de: ' + names.join(', ')); } }
  };
...
function renderProgreso(){
  ...
  const nota = document.getElementById('progreso-nota'); if(nota){
    let msg=''; if (porcentaje === 100) msg='Felicitaciones, podes anotarte en el 108 A';
    else if (porcentaje >= 75) msg='Podes anotarte en el listado 108 b Item 4';
    else if (porcentaje >= 50) msg='Podes anotarte en el listado 108 b Item 5';
    else if (porcentaje > 25) msg='Podes anotarte en el listado de Emergencia';
    else msg='Seguí sumando materias para habilitar listados.';
...
function setupCollapsibles(){
  document.querySelectorAll('.collapse-toggle').forEach(btn0 => {
    const targetId0 = btn0.getAttribute('data-target'); const panel0 = document.getElementById(targetId0);
    if(panel0 && panel0.classList.contains('collapsed')){ btn0.textContent = btn0.textContent.replace('▾','▸'); }
  });
  document.querySelectorAll('.collapse-toggle').forEach(btn => {
    const targetId = btn.getAttribute('data-target'); const panel = document.getElementById(targetId);
    btn.addEventListener('click', ()=>{ panel.classList.toggle('collapsed'); btn.textContent = btn.textContent.includes('▾') ? btn.textContent.replace('▾','▸') : btn.textContent.replace('▸','▾'); });
  });
}
